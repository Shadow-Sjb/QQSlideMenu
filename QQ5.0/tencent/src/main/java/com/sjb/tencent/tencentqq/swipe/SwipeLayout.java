...
public class SwipeLayout extends FrameLayout implements SwipeLayoutInterface{
	...
	private ShowEdge mShowEdge = ShowEdge.Right;//默认方向为右
	...
	public interface SwipeListener{
		void onClose(SwipeLayout swipeLayout);//关闭
		void onOpen(SwipeLayout swipeLayout);//打开
		void onStartClose(SwipeLayout swipeLayout);//开始关闭
		void onStartOpen(SwipeLayout swipeLayout);//开始打开
	}
	...
	protected void onFinishInflate() {
		...
		if(getChildCount() != 2){
			throw new IllegalStateException("2 views in SwipeLayout");
		}
...
//			Log.d(TAG, "xvel: " + xvel + " mShowEdge: " + mShowEdge);
	...
	public Status getCurrentStatus() {
		int left = getFrontView().getLeft();//获取左边距
		if(left == 0){
			return Status.Close;//关闭
		}
		if((left == 0 - mDragDistance) || (left == mDragDistance)){
			return Status.Open;//打开
		}
		return Status.Swiping;//滑动中
	}
	...
	private void updateStatus(boolean isNotify) {
		Status lastStatus = mStatus;//记录之前状态
		Status status = getCurrentStatus();//获取当前状态

		if (status != mStatus) {//状态改变了
			mStatus = status;//更新状态
			if (!isNotify || mSwipeListener == null) {//监听器为空或者不回调接口，直接返回
				return;
			}
			if (mStatus == Status.Open) {
				mSwipeListener.onOpen(this);//打开的接口回调
			} else if (mStatus == Status.Close) {
				mSwipeListener.onClose(this);//关闭的接口回调
			} else if (mStatus == Status.Swiping) {
				if (lastStatus == Status.Open) {
					mSwipeListener.onStartClose(this);//开始关闭的接口回调
				} else if (lastStatus == Status.Close) {
					mSwipeListener.onStartOpen(this);//开始打开的接口回调
				}
			}
		} else {
			mStatus = status;//状态没有改变，保持原值
		}
	}
