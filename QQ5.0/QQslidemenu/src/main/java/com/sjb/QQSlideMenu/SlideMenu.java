...
public class SlideMenu extends FrameLayout{
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		width = getMeasuredWidth();//自己的宽度
		dragRange = width*0.6f;//滑动的最大范围
	}
...
//			float intFraction = mainView.getLeft()/(int)dragRange;//适配屏幕
			float v = ((int) dragRange) / dragRange;
			...
			}else if (fraction == ((int) dragRange) / dragRange && currentState!=DragState.Open) {
				//更改状态为打开，并回调打开的方法
